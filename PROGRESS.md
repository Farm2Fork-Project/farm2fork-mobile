...
| **farm2fork-web** (Next.js buyer app) | Existing-buyer login, marketplace/product detail, farmer-grouped cart/checkout, orders and payment-status views now use the backend API; standalone Docker runtime is defined | Browser/user-flow smoke remains open; temporary `sessionStorage` token is not the final session security model |
...
- **Still open in this slice** — real JazzCash/Stripe and provider signature validation; mobile polling after a real-gateway redirect; a normal payment/shipment-flow Fabric smoke; web API integration; automatic refund restocking. The full backend e2e suite remains unverified because its Jest runtime currently conflicts with the Fabric SDK's ESM dependency path.
...
**Slice 5 — Web buyer API integration (implementation complete; local-only).**

- **Web buyer path** — branch `feature/web-buyer-api-integration`: `455632f`, `44cacce`, `e213c04`, `cd28264`, `ba4de79`, `1e2f489` add exact backend DTO contracts and product mapping without fabricated farmer data; a versioned `sessionStorage` buyer session; a bearer-token request client that clears on `401`; buyer-only repository methods; farmer-grouped cart helpers; and a Vitest/DOM test harness. The buyer UI now authenticates an existing buyer with `/auth`, validates the saved session with `/auth/me`, reads live marketplace/products, submits one checkout order per farmer group, initiates a pending payment per resulting order, and reads real orders/payment statuses. The former mock buyer login, products, cart checkout, and payment simulation are not used by this path. `pnpm test` passed (**11 tests**) and `pnpm run build` passed with the standalone Next output.
- **Web Docker runtime** — the web `Dockerfile`, Compose definition, `.dockerignore`, and runtime documentation use an explicitly required `NEXT_PUBLIC_API_BASE_URL`, build the standalone Next server, expose web on port `3001`, and do not use `env_file` or carry secrets into the image. `NEXT_PUBLIC_API_BASE_URL=http://localhost:3000/api docker compose config` passed. A later image-build retry downloaded about **442/445** packages but `pnpm install --frozen-lockfile` failed after roughly 4m 23s with registry fetch timeout error `23`; `docker image inspect farm2fork-web-web` confirms no image was created. This is a Docker-to-npm-registry network failure before the Next build, not an application build result. Backend Compose must set `CORS_ORIGIN=http://localhost:3001` for this runtime.
