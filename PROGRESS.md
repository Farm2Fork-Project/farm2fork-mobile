...
- **Web Docker runtime** — the web `Dockerfile`, Compose definition, `.dockerignore`, and runtime documentation use an explicitly required `NEXT_PUBLIC_API_BASE_URL`, build the standalone Next server, expose web on port `3001`, and do not use `env_file` or carry secrets into the image. `NEXT_PUBLIC_API_BASE_URL=http://localhost:3000/api docker compose config` passed. The image build was started but did not complete in the available tool session while dependencies were downloading, so no successful image/container run is claimed. Backend Compose must set `CORS_ORIGIN=http://localhost:3001` for this runtime.
