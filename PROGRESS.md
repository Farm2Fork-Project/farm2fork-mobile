...
- **Web transporter and buyer flow** — `feature/web-shipment-self-claim`: `ae39b4d`, `dcf63a0`, `8ed9bf1`, `b1f6b8f` replace seeded transporter shipments with privacy-preserving available deliveries, atomic `{ orderId }` claims, server-owned full routes after claim, required-note delivery transitions, and status-history timelines. Buyer real orders load scoped shipments and join only matching `orderId` records; no farmer mock order data was connected. `CI=true pnpm test` passed (**16 files, 30 tests**), `CI=true pnpm run build` passed, and `NEXT_PUBLIC_API_BASE_URL=http://localhost:3000/api docker compose build` passed.
