...
| Repo | State | Notes |
|------|-------|-------|
| **farm2fork-backend** (Nest.js) | Auth + Profiles + 17 schemas + **Marketplace** + **Orders** + persisted payment initiation/read/simulated settlement | Payment refund still returns a mock DTO; blockchain/transport/loan/community/notification/ai are otherwise schema-only scaffolds |
...
- **Backend payment write/read path** — `15f585a`, `f8355f6`, `4b87ae3`, `021aa91`: buyer-owned pending orders create/reuse a persisted payment whose amount comes from `Order.grandTotal`; buyer/admin list and individual reads are now database-backed and ownership-scoped. The guarded local simulator settles success idempotently in a MongoDB transaction, decrements stock once, marks zero stock `sold_out`, sets the order `paid`, and creates one pending blockchain outbox record. A public webhook now returns 501 until a real provider signature design exists. `pnpm run build` and `pnpm test` passed (62 tests).
