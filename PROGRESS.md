...
| **farm2fork-web** (Next.js buyer prototype) | Buyer API contracts, temporary browser session adapter, HTTP client, buyer repository, and farmer-grouped cart foundation committed; UI remains mock-backed | React screen conversion, Docker runtime, and browser tests remain open |
...
- **Web foundation** — branch `feature/web-buyer-api-integration`: `455632f`, `44cacce`, `e213c04` add exact backend DTO contracts and product mapping without fabricated farmer data; a versioned `sessionStorage` buyer session; a request client that adds a bearer token, clears session on `401`, and normalizes non-JSON failures; buyer-only repository methods; and cart helpers that group by farmer and omit both farmer IDs and client-calculated totals from order payloads. The pure contract/session/client/repository/cart tests pass with Node's built-in TypeScript runner. The planned Vitest/DOM harness is not yet installed: repeated pnpm network-resolution attempts ended before changing the manifest or lockfile and left the local dependency links unreliable, so full web build/lint/browser verification is pending a successful pnpm dependency restore.
...
- **Still open in this slice** — restore the web pnpm dependency tree and add the planned Vitest/DOM harness; replace the buyer mock login, marketplace, product detail, cart, checkout, and orders/payment screens; Dockerize the web runtime; and run user-owned browser/device smoke checks. The security target remains HTTP-only cookie sessions, not the temporary `sessionStorage` token used for the first integration model.
