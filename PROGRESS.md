...
- **Web buyer path** — branch `feature/web-buyer-api-integration`: `455632f`, `44cacce`, `e213c04`, `cd28264`, `ba4de79`, `1e2f489`, `bce5220`, `847dfc0`, `3d8f5af`, `30e3cf0` add exact backend DTO contracts and product mapping without fabricated farmer data; a versioned `sessionStorage` buyer session; a bearer-token request client that clears on `401`; buyer-only repository methods; farmer-grouped cart helpers; and a Vitest/DOM test harness. Buyer registration now posts the backend-required business name, business type, CNIC, email, strong password, and optional phone to `/auth/register/buyer`; it accepts only a buyer response before saving the returned session. Selecting Buyer from the sign-up role screen opens this real form, rather than redirecting to login. The legacy `/login` mock-credential page now redirects to the real `/?auth=login` entry point. The buyer UI validates saved sessions with `/auth/me`, reads live marketplace/products, submits one checkout order per farmer group, initiates a pending payment per resulting order, and reads real orders/payment statuses. Mock buyer login, products, cart checkout, and payment simulation are not used by this path. `pnpm test` passed (**14 tests**) and `pnpm run build` passed with the standalone Next output.
