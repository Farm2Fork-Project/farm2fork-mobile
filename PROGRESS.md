...
| Repo | State | Notes |
|------|-------|-------|
| **farm2fork-backend** (Nest.js) | Auth + Profiles + 17 schemas + **Marketplace** + **Orders** + persisted payment initiation/simulated settlement | Payment list/read/refund still return mock DTOs; blockchain/transport/loan/community/notification/ai are otherwise schema-only scaffolds |
...
- **Backend payment write path** — `15f585a`, `f8355f6`, `4b87ae3`: buyer-owned pending orders create/reuse a persisted payment whose amount comes from `Order.grandTotal`. The guarded local simulator settles success idempotently in a MongoDB transaction, decrements stock once, marks zero stock `sold_out`, sets the order `paid`, and creates one pending blockchain outbox record. A public webhook now returns 501 until a real provider signature design exists. `pnpm run build` and `pnpm test` passed (59 tests).
- **Mobile flow** — branch `feature/payment-mobile`: `be0ff8f`, `5574a6e`, `025b460` add typed payment API/mock repositories, a payment controller, a localized buyer status screen, and checkout-to-payment navigation. The test-completion action is visible only in mock mode or with `PAYMENT_SIMULATOR_ENABLED=true`; normal API builds create a pending payment and do not call the simulator. `flutter analyze` and `flutter test` passed (59 tests).
- **Still open in this slice** — real JazzCash/Stripe and provider signature validation; Atlas Compose/device smoke test; transaction integration test against a MongoDB replica set; persisted payment `GET`/list/refund behavior; mobile polling after a real-gateway redirect; Fabric Gateway SDK/retry worker; web API integration; automatic refund restocking.
